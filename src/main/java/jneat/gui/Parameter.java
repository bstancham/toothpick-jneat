...
public class Parameter extends JPanel implements ActionListener, ListSelectionListener {
    ...
    String paramsTitlePrefix = "J N E A T parameters: ";
    TitledBorder paramsTitleBorder;
    ...
    public Parameter(JFrame _f) {
        ...
        paramsTitleBorder = BorderFactory.createTitledBorder(paramsTitlePrefix);

        p3.setBorder(BorderFactory.createCompoundBorder(paramsTitleBorder, 
                                                        BorderFactory.createEmptyBorder(10, 10, 2, 2))); 
    ...
    public void loadDefaultJneatParameters() {
        loadJneatParameters(EnvRoutine.getJneatParameter());
    }
    
    public void loadJneatParameters(String filePath) {
        String xret = null;
        logger.sendToLog(" loading parameters file: " + filePath);

        File f = new File(filePath);
        if (!f.exists())
            logger.sendToLog(" WARNING: file does not exist: " + filePath);
            
        netx = new Neat();
        netx.initbase();
        // String name = EnvRoutine.getJneatParameter();
        boolean rc = netx.readParam(filePath);
		 
        if (rc) {
            xret = new String("  OK");
            paramsTitleBorder.setTitle(paramsTitlePrefix + f.getName());
        } else {
            xret = new String("  *ERROR*");
        }
        ...
        ListSelectionModel lsm = jtable1.getSelectionModel();
        lsm.addListSelectionListener(this);
        logger.sendToLog(xret + ": loading jneat parameter file - " + filePath);
        logger.sendToStatus("READY");
    }
